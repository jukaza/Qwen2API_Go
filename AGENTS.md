# Quy ước Lập trình dành cho AI Agent (AGENTS.md)
...
*   **Kỷ luật cấm tự ý sử dụng Tool khi chưa xác nhận hiểu yêu cầu:** Khi người dùng đưa ra một yêu cầu mới, Agent CHỈ được phép phản hồi để làm rõ hoặc xác nhận sự hiểu biết. Tuyệt đối KHÔNG được tự ý sử dụng bất kỳ công cụ (tool) nào (bao gồm tìm kiếm, đọc/sửa file, chạy lệnh, lên kế hoạch) cho đến khi người dùng xác nhận rõ ràng rằng Agent đã hiểu đúng 100% yêu cầu.
